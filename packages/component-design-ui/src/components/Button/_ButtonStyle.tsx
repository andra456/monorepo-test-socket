...
const getPaddingBlock = (size: ButtonSize, variant: ButtonVariant, iconOnly: boolean) => {
  let paddingBlock: { [s in ButtonSize]: { [v in ButtonVariant]: any } } = {
    lg: {
      primary: {
        noIcon: 8,
        iconOnly: 12,
...
const getMinWidth = (size: ButtonSize, iconOnly: boolean) => {
  let minWidth: { [s in ButtonSize]: number } = {
    lg: 120,
    md: 120,
    sm: 88,
  };
...
const getFontSize = (size: ButtonSize) => {
  let fontSize: { [s in ButtonSize]: number } = {
    lg: 16,
    md: 14,
    sm: 12,
  };
